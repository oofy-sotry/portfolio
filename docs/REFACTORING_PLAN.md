...
- [x] 챗봇 라우트에 `@login_required` 추가
- [x] 시스템 프롬프트에 "포트폴리오 어시스턴트" 역할 + "사용자 본인으로 답변 금지" 제약 명시
- [x] 사용자 프로필 정보(이름, 직책, 기술 스택, 경력)를 LLM 프롬프트에 컨텍스트로 주입
- [x] 모든 검색 모드(FAQ/검색/AI)에 프로필 컨텍스트 적용
