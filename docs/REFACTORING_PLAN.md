...
### 1-1. docker-compose.llm.yml 삭제
- [ ] `docker-compose.llm.yml` 삭제
  - MariaDB 10.11 기반의 구버전 설정 (현재 MySQL 8.0 사용)
  - Keycloak 설정 누락
  - GPU LLM 컨테이너 방식은 사용하지 않음 (향후 API 방식으로 전환)

### 1-2. 검색 기능 정상화
- [ ] `search/advanced.html` 템플릿 생성
  - `search.py:65`에서 렌더링하지만 템플릿이 존재하지 않음
  - ES 기반 고급 검색 UI (카테고리 필터, 태그, 날짜 범위, 정렬)
  - 검색어 자동완성, 인기 검색어 표시
- [ ] `base.html` 네비게이션에 검색 링크 추가/확인
